...
class TesseramentoController(http.Controller):
    ...
    def tesseramento_comuni_search(self, term="", **kw):
        ...
        results = [
            {
                "id": c.id,
                "text": f"{c.name} ({c.provincia})" if c.provincia else c.name,
            }
            for c in comuni
        ]
    ...
    @http.route(
        "/tesseramento", type="http", auth="public", website=True, methods=["GET"]
    )
    def tesseramento_form(self, **kw):
        ...
        is_logged_in = (
            user and not user._is_public() and user.id != request.website.user_id.id
        )

        # Ottieni associazioni attive che consentono registrazione pubblica
        associazioni = (
            request.env["associazione.culturale"]
            .sudo()
            .search(
                [("attivo", "=", True), ("consenti_registrazione_pubblica", "=", True)]
            )
        )
    ...
    def tesseramento_submit(self, **post):
        ...
        try:
            ...
            no_codice_fiscale = post.get("no_codice_fiscale") in (
                "on",
                "1",
                "true",
                "yes",
            ...
            codice_fiscale_raw = (
                post.get("codice_fiscale", "").strip() if not no_codice_fiscale else ""
            )
            if codice_fiscale_raw:
                # Rimuovi tutti i caratteri non alfanumerici e converti in maiuscolo
                codice_fiscale = "".join(
                    c for c in codice_fiscale_raw.upper() if c.isalnum()
                )
            else:
                codice_fiscale = ""
            ...
            comune_nascita_id = post.get("comune_nascita_id")
            if comune_nascita_id:
                comune_obj = (
                    request.env["res.comune"].sudo().browse(int(comune_nascita_id))
                )
                if comune_obj.exists():
                    luogo_nascita = comune_obj.name
            ...
            if not associazione_id:
                return request.render(
                    "associazioni_culturali.tesseramento_error",
            ...
            if not user_email:
                return request.render(
                    ...
                    {
                        "error": _(
                            "Il tuo account non ha un'email. Aggiungila al profilo prima di richiedere la tessera."
                        )
                    },
                )
            ...
            if (
                not nome_legale
                and not cognome_legale
                and user.partner_id
                and user.partner_id.name
            ):
            ...
            associato_vals = {
                "email": user_email,
                "user_id": user.id,
                "nome_legale": nome_legale
                or (associato.nome_legale if associato else False),
                "cognome_legale": cognome_legale
                or (associato.cognome_legale if associato else False),
                "nome_elezione": nome_elezione
                or (associato.nome_elezione if associato else False),
                "no_codice_fiscale": no_codice_fiscale,
                "codice_fiscale": codice_fiscale
                or (
                    associato.codice_fiscale
                    if associato and not no_codice_fiscale
                    else False
                )
                if not no_codice_fiscale
                else False,
                "data_nascita": data_nascita
                or (associato.data_nascita if associato else False),
                "luogo_nascita": luogo_nascita
                or (associato.luogo_nascita if associato else False),
                "comune_nascita_id": int(comune_nascita_id)
                if comune_nascita_id
                else (associato.comune_nascita_id.id if associato else False),
                ...
                if state_id
                else (associato.state_id.id if associato else False),
                "country_id": int(country_id)
                if country_id
                else (associato.country_id.id if associato else False),
            ...
            if (
                ...
                email_normalized = (
                    user.partner_id.email.lower().strip()
                ...
                if email_normalized:
                    ...
                    if mailing_list_ids:
                        ...
                        valid_lists = (
                            ...
                            .sudo()
                            .browse(list_ids)
                            .filtered(lambda l: l.exists() and l.active and l.is_public)
                        )
                        ...
                        if valid_lists:
                            if existing_contact:
                                ...
                                new_lists = [
                                    lid
                                    for lid in valid_lists.ids
                                    if lid not in current_lists
                                ]
                                if new_lists:
            ...
            tesseramento_pending = (
                request.env["tesseramento.pending"]
                .sudo()
                .create(
                    {
            ...
            for p in providers:
                if (
                    ...
                    or piano.currency_id.id in p.available_currency_ids.ids
                ):
                    provider = p
                    break
            ...
            if not provider:
                ...
                return request.render(
                    ...
                    {
                        "error": _(
                            "Nessun metodo di pagamento disponibile per questa valuta. "
                        ...
                        .create(
                            {
                                ...
                                "currency_id": piano.currency_id.id,
                                "partner_id": user.partner_id.id,
                                "description": _("Tesseramento")
                                + f" - {associazione.name} - {piano.name}",
                            }
                        )
                    )
                    if payment_link_wizard.link:
                        tx.return_url = (
                            f"/tesseramento/payment/return?reference={tx.reference}"
                        )
            ...
            try:
                ...
                payment_link = (
                    provider._get_landing_route(tx)
                    if hasattr(provider, "_get_landing_route")
                    else None
                )
                if payment_link:
            ...
            return request.render(
                ...
                {
                    "error": _(
                        "Impossibile generare il link di pagamento. Contatta l'amministratore."
                    )
                },
            )
            ...
            return request.render(
                "associazioni_culturali.tesseramento_error",
                {"error": _("Si è verificato un errore: %s") % str(e)},
            )

    @http.route(
        "/tesseramento/success", type="http", auth="user", website=True, methods=["GET"]
    )
    def tesseramento_success_page(self, **kw):
        ...
        if tesseramento and tesseramento.tessera_id:
            return request.render(
                "associazioni_culturali.tesseramento_success",
                {
    ...
    def tesseramento_payment_return(self, **kw):
        ...
        tx_reference = request.params.get("reference") or request.params.get(
            "tx_reference"
        )
        if not tx_reference:
        ...
        tx = (
            ...
            .sudo()
            .search([("reference", "=", tx_reference)], limit=1)
        )
        ...
        tesseramento = (
            request.env["tesseramento.pending"]
            .sudo()
            .search([("transaction_id", "=", tx.id)], limit=1)
        )
    ...
    def my_tessere(self, **kw):
        ...
        piani = request.env["piano.tesseramento"].sudo().search([("attivo", "=", True)])
        associazioni = (
            request.env["associazione.culturale"].sudo().search([("attivo", "=", True)])
        )
    ...
    @http.route(
        "/my/associato/reclama", type="http", auth="user", website=True, methods=["GET"]
    )
    def associato_reclama_page(self, **kw):
        ...
        if not user_email:
            return request.render(
                "associazioni_culturali.associato_reclama",
                {
                    "error": _(
                        "Il tuo account non ha un'email. Aggiungila al profilo."
                    ),
                    "associati_da_reclamare": request.env["associato"],
                },
            )
        associati_da_reclamare = (
            request.env["associato"]
            .sudo()
            .search(
                [
                    ("email", "=", user_email),
    ...
    @http.route(
        "/my/associato/reclama/<int:associato_id>",
        type="http",
        auth="user",
        website=True,
        methods=["POST"],
    ...
    def rinnova_tessera_form(self, **kw):
        ...
        if not associati:
            return request.redirect(
                "/my/associato/reclama?redirect=/my/tessere/rinnova"
            )
        ...
        try:
            ...
            if not associati:
                return request.render(
                    ...
                    {
                        "error": _(
                            "Nessun profilo associato collegato. Reclama un profilo dalla tua email."
                        )
                    },
                )
            associato = associati[0]
            ...
            if comune_nascita_id:
                associato_vals["comune_nascita_id"] = int(comune_nascita_id)
            if street:
